...
![image-20190925215603036](RabbitMQ.assets/image-20190925215603036-9419777.png)
...
![image-20200311182438041](RabbitMQ.assets/image-20200311182438041.png)

### 2.2 RabbitMQ 的安装

#### 2.2.1 下载

`官网下载地址`: https://www.rabbitmq.com/download.html![image-20190925220115235](RabbitMQ.assets/image-20190925220115235.png)

> `最新版本`: 3.7.18

#### 2.2.2 下载的安装包![image-20190925220343521](RabbitMQ.assets/image-20190925220343521.png)
...
![image-20190925222230260](RabbitMQ.assets/image-20190925222230260-3836271.png)

将上图中配置文件中红色部分去掉`%%`,以及最后的`,`逗号 修改为下图:

![image-20190925222329200](RabbitMQ.assets/image-20190925222329200-3836312.png)
...
![image-20190925222743776](RabbitMQ.assets/image-20190925222743776-3836511.png)
...
```
 ...
 ![image-20190926194738708](RabbitMQ.assets/image-20190926194738708-3836601.png)
...
![image-20190926194954822](RabbitMQ.assets/image-20190926194954822-3836665.png)
...
![image-20191126162026720](RabbitMQ.assets/image-20191126162026720.png)
...
![image-20191126162617280](RabbitMQ.assets/image-20191126162617280.png)
...
```
 ...
 ![image-20191126163023153](RabbitMQ.assets/image-20191126163023153.png)
...
点击添加好的虚拟主机：
 ...
 ![image-20191126163506795](RabbitMQ.assets/image-20191126163506795.png)

进入虚拟机设置界面:

![image-20191126163631889](RabbitMQ.assets/image-20191126163631889.png)
...
![image-20200312140114784](RabbitMQ.assets/image-20200312140114784.png)

### 4.1 RabbitMQ支持的消息模型

![image-20191126165434784](RabbitMQ.assets/image-20191126165434784.png)

![image-20191126165459282](RabbitMQ.assets/image-20191126165459282.png)

![image-20210617194948140](RabbitMQ.assets/image-20210617194948140.png)
...
![image-20191126165840602](RabbitMQ.assets/image-20191126165840602.png)
...
```java
  ...
  //参数2: 用来定义数据是否持久化  true持久化队列  false不持久化队列
  //参数3:是否独占队列  true独占队列  false不独占
  //参数4:队列是否自动删除（消息为空且没有被监听时）  true自动删除队列  false不自动删除队列
  //参数5:其他属性
  channel.queueDeclare("hello",true,false,false,null);
  //发布消息
  //参数1:交换机名称 参数2:队列名称 参数3:传递消息的额外设置 参数4:消息的具体内容
  //参数3:如果需要将消息队列中的数据持久化吗，那么这里设置为MessageProperties.PERSISTENT_TEXT_PLAIN
...
- 关闭消息的自动确认,开启手动确认消息
  ...
  ![image-20200314230412178](RabbitMQ.assets/image-20200314230412178.png)

  ![image-20200314230423280](RabbitMQ.assets/image-20200314230423280.png)
...
```java
//声明交换机
//参数1:交换机名称  参数2:交换机的类型，当交换机的类型是fanout的时候，表示是一种广播类型
...
![image-20200315180728035](RabbitMQ.assets/image-20200315180728035.png)

----

### 4.6 第四种模型(Routing)

#### 4.6.1 Routing 之==订阅模型==-Direct(直连)
...
**注：实际订单系统中，用户下完订单之后，应该返回用户订单处理中，此时用户可以选择等待下单成功，也可以选择逛其他页面，过一段时间后来看订单是否下单成功。因为直接返回下单成功，后续库存系统查库后发现没有库存商品后，就会出现订单被取消后者长时间不发货的情况！**
...
```bash
docker run --name rabbitmq -d -p 15672:15672 -p 5672:5672 -e RABBITMQ_DEFAULT_USER=liyang -e RABBITMQ_DEFAULT_PASS=liyang  rabbitmq:management
```
...
==@Test不支持多线程吗？==
 ...
 就如正常运行一样，测试的主线程并不会等待子线程执行完毕，要想等待子线程执行完毕，可以让主线程睡眠一会会这使用countDownLatch，基本原理就是让主线程等待。
